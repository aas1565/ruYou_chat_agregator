...
export function MessageBubble({ message }: { message: MessageDto }) {
  if (message.senderType === "SYSTEM") {
    return (
      <div className="flex justify-center py-1">
        <p className="max-w-[80%] rounded-full bg-slate-100 px-3 py-1 text-center text-xs text-muted">
          {message.text}
        </p>
      </div>
    );
  ...
  return (
    <div className={cn("flex flex-col", ALIGN[message.senderType])}>
      <div
        className={cn(
          "max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-5 shadow-sm",
          message.senderType === "CLIENT" && "rounded-tl-md bg-white text-ink",
          message.senderType === "OPERATOR" && "rounded-tr-md bg-accent text-white",
          message.senderType === "AI" && "rounded-tr-md bg-slate-800 text-white",
        )}
      >
        <p className={cn("mb-1 text-[11px] font-medium", outgoing ? "text-white/80" : "text-muted")}>
