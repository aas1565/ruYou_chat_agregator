...
import { formatDate, formatDateTime, formatDateTimeInZone, fullName } from "@/lib/utils";
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 md:px-8">
        ...
        <section className="rounded-xl border border-border bg-surface p-5">
          ...
          <div className="mt-3 space-y-2">
            {client.identities.map((identity) => (
              <div key={identity.id} className="flex items-center justify-between rounded-lg bg-surface-2 px-3 py-2">
      ...
      <section className="mt-6 rounded-xl border border-border bg-surface p-5">
        ...
        <div className="mt-3 divide-y divide-border">
          {client.conversations.map((conversation) => (
            <Link key={conversation.id} href={`/inbox?c=${conversation.id}`} className="flex items-center justify-between gap-4 py-3 hover:bg-surface-2">
              <div>
      ...
      <section className="mt-6 rounded-xl border border-border bg-surface p-5">
        ...
        <div className="mt-3 space-y-2">
          ...
          {client.leads.map((lead) => (
            <Link key={lead.id} href={`/leads/${lead.id}`} className="block rounded-lg border border-border px-3 py-2 hover:bg-surface-2">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <section className="rounded-xl border border-border bg-surface p-5">
          ...
          <div className="mt-3 space-y-2">
            {client.appointments.length === 0 ? <p className="text-sm text-muted">Записей нет</p> : null}
            {client.appointments.map((appointment) => (
              <Link key={appointment.id} href={`/appointments?date=${appointment.startAt.slice(0, 10)}`} className="block rounded-lg border border-border px-3 py-2 hover:bg-surface-2">
                <div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">{appointment.service.name}</p><Badge tone={appointment.status === "CANCELLED" ? "danger" : appointment.status === "COMPLETED" ? "neutral" : "success"}>{appointment.status}</Badge></div>
                <p className="mt-1 text-xs text-muted">{formatDateTimeInZone(appointment.startAt, appointment.timezone)} · {appointment.staff.name}</p>
              </Link>
            ))}
          </div>
        </section>
