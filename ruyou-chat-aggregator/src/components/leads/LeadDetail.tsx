...
export function LeadDetail({ id }: { id: string }) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 md:px-8">
      ...
      <section className="mt-6 rounded-xl border border-border bg-surface p-5">
        <h2 className="text-sm font-semibold">Связанная запись</h2>
        {data.appointment ? (
          <div className="mt-3 rounded-lg bg-surface-2 px-3 py-3 text-sm">
            <p className="font-medium">{data.appointment.service.name} · {data.appointment.staff.name}</p>
            <p className="mt-1 text-muted">{new Intl.DateTimeFormat("ru-RU", { timeZone: data.appointment.timezone, dateStyle: "medium", timeStyle: "short" }).format(new Date(data.appointment.startAt))}</p>
            <Link href={`/appointments?date=${data.appointment.startAt.slice(0, 10)}`} className="mt-2 inline-block text-accent hover:text-accent-hover">Открыть в расписании</Link>
          </div>
        ) : <p className="mt-3 text-sm text-muted">Запись не создана</p>}
      </section>
