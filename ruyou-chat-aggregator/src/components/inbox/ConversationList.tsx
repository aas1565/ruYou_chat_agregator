...
export function ConversationList({
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col border-r border-border bg-surface/80">
      <div className="space-y-3 border-b border-border bg-surface/70 p-4 backdrop-blur">
        <div>
          <h1 className="text-base font-semibold">Обращения</h1>
          <p className="text-xs text-muted">Все каналы в одном рабочем потоке</p>
        </div>
        <Input
          value={q}
        ...
        <div className="flex gap-1 rounded-xl bg-surface-3/80 p-1">
          {FILTERS.map((item) => (
            <button
              key={item.id}
              ...
              className={cn(
                ...
                filter === item.id ? "bg-surface text-ink shadow-none" : "text-muted hover:text-ink",
              )}
            >
              {item.label}
            </button>
