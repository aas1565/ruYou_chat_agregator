...
async function main() {
  await prisma.conversationBookingState.deleteMany();
  ...
  const services = await Promise.all([
    prisma.service.create({ data: { name: "Диагностика", description: "Комплексная диагностика автомобиля.", durationMinutes: 40, priceText: "от 1 800 ₽" } }),
    prisma.service.create({ data: { name: "Шиномонтаж", description: "Снятие, установка и балансировка колёс.", durationMinutes: 60, priceText: "от 1 500 ₽" } }),
    prisma.service.create({ data: { name: "Замена масла", description: "Замена масла и фильтров.", durationMinutes: 45, priceText: "от 2 900 ₽" } }),
    prisma.service.create({ data: { name: "Развал-схождение", description: "Регулировка углов установки колёс.", durationMinutes: 60, priceText: "от 2 600 ₽" } }),
  ]);
  const [diagnostics, tire, oil, alignment] = services;
  const staff = await Promise.all([
    prisma.staffMember.create({ data: { name: "Иван Петров", specialization: "Автомеханик", services: { create: [{ serviceId: diagnostics.id }, { serviceId: tire.id }, { serviceId: oil.id }] } } }),
    prisma.staffMember.create({ data: { name: "Сергей Волков", specialization: "Диагност", services: { create: [{ serviceId: diagnostics.id }, { serviceId: alignment.id }] } } }),
    prisma.staffMember.create({ data: { name: "Ольга Соколова", specialization: "Мастер шиномонтажа", services: { create: [{ serviceId: tire.id }, { serviceId: alignment.id }] } } }),
  ]);
  for (const member of staff) {
    for (let dayOfWeek = 1; dayOfWeek <= 5; dayOfWeek += 1) await prisma.staffWorkingHours.create({ data: { staffId: member.id, dayOfWeek, startTime: "09:00", endTime: "18:00", isWorkingDay: true } });
    await prisma.staffWorkingHours.create({ data: { staffId: member.id, dayOfWeek: 6, startTime: "10:00", endTime: "16:00", isWorkingDay: true } });
    await prisma.staffWorkingHours.create({ data: { staffId: member.id, dayOfWeek: 0, startTime: "09:00", endTime: "09:00", isWorkingDay: false } });
  }
  const exceptionDate = new Date(Date.now() + 6 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  await prisma.scheduleException.create({ data: { staffId: staff[1].id, date: exceptionDate, type: "BLOCKED_TIME", startTime: "13:00", endTime: "14:00", reason: "Обеденный перерыв" } });
  const futureStart = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
  futureStart.setMinutes(0, 0, 0);
  const futureEnd = new Date(futureStart.getTime() + diagnostics.durationMinutes * 60_000);
  const ivanConversation = await prisma.conversation.findFirst({ where: { clientId: ivan.id, channel: "TELEGRAM" }, orderBy: { createdAt: "asc" } });
  const ivanLead = ivanConversation ? await prisma.lead.findFirst({ where: { conversationId: ivanConversation.id, funnel: "SERVICE" } }) : null;
  await prisma.appointment.create({ data: { clientId: ivan.id, serviceId: diagnostics.id, staffId: staff[1].id, startAt: futureStart, endAt: futureEnd, leadId: ivanLead?.id, conversationId: ivanConversation?.id, source: "SEED", comment: "Демо-запись", history: { create: { action: "CREATED", toStartAt: futureStart, newStatus: "CONFIRMED", actorType: "SYSTEM" } } } });
  const cancelledStart = new Date(futureStart.getTime() + 3 * 60 * 60 * 1000);
  const cancelledEnd = new Date(cancelledStart.getTime() + tire.durationMinutes * 60_000);
  await prisma.appointment.create({ data: { clientId: anna.id, serviceId: tire.id, staffId: staff[0].id, startAt: cancelledStart, endAt: cancelledEnd, status: "CANCELLED", source: "SEED", cancellationReason: "Демо отмены", history: { create: { action: "CANCELLED", fromStartAt: cancelledStart, oldStatus: "CONFIRMED", newStatus: "CANCELLED", actorType: "SYSTEM", reason: "Демо отмены" } } } });
